...
std::shared_ptr<CWallet> LoadWalletInternal(WalletContext& context, const std::string& name, std::optional<bool> load_on_start, const DatabaseOptions& options, DatabaseStatus& status, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    try {
        ...
        if (!wallet->IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS))
            warnings.push_back(_("Wallet loaded successfully. This is a Legacy Wallet that needs to be upgraded to be used in Riecoin 23.04+. They can be migrated to a Descriptor Wallet with migratewallet."));
        else
            warnings.push_back(_("Wallet loaded successfully. This is already a Descriptor Wallet usable in Riecoin 23.04+, no action required!"));
